...
import { LoadingSpinner, ErrorMessage, LoadingOverlay } from "@/components/ui/loading-states"
import { Img } from "@/components/utility/img"
import { useAvmData, useAvmSubCategories, useBrands } from "@/hooks/useAvmQueries"
import { processFilters, getFloorDisplayName, type FilterData } from "@/lib/utils/filter-utils"
...
export function FilterableContent({ brands }: FilterableContentProps) {
  ...
  const filteredBrands = brandsQuery.data?.data?.items || brands
  ...
  const handleFilter = (filterData: FilterData) => {
    setFilters(processFilters(filterData))
  }

  const handleCategoryChange = () => {
    // React Query will automatically handle subcategories fetching
  }
  ...
  return (
    <div className="section-container min-h-screen">
      ...
      {loading && <LoadingSpinner message="Veriler yükleniyor..." />}

      {/* Results count - Only show when not in initial loading */}
      {/* {!loading && (
        <motion.div
          className="mb-6"
        ...
        >
          <p className="text-sm text-gray-600 font-suisse-intl">
            {filteredBrands.length} sonuç bulundu
            {brandsQuery.isLoading && " (filtreleme yapılıyor...)"}
          </p>
        </motion.div>
      )} */}
      ...
      {!loading && (
        <>
          {/* Loading overlay for brands grid */}
